...
import { am as toRefs, B as ref, f as computed, C as onMounted, H as onUnmounted, E as watch, o as openBlock, k as createElementBlock, z as mergeProps } from "./index.2c4f64fa.js";
function u(t) {
      ...
      d2.forEach(function(t3, e3) {
        for (var s4 = t3.length, l3 = 0; l3 < s4; l3++) {
          ...
          if (l3 === s4 - 1) {
            ...
            var f3 = o2 ? "bottom" : "right", d3 = i2 ? 0 : s4 - 1, v3 = 1e3 - h2[e3][d3];
            p3 = (n2 && !o2 ? 100 : 0) + p3 / s4 - v3, a2[c3].innerHTML = m2[e3].join(r), a2[c3].style.display = "block", a2[c3].style[f3] = p3 + "%", u2.value.tooltipHidden.split(" ").forEach(function(t4) {
...
var x = d(h(function(t, e) {
  !function(t2) {
    ...
    function st(e3, r2, s2) {
      var u3, d3, S2, w3, P3, E2 = b2(), N2 = x2() && y2(), C2 = e3, O2 = r2.spectrum, k2 = [], V2 = [], A2 = [], M2 = 0, j2 = {}, U2 = e3.ownerDocument, D2 = r2.documentElement || U2.documentElement, T2 = U2.body, z2 = U2.dir === "rtl" || r2.ort === 1 ? 0 : 100;
      function F2(t3, e4) {
      ...
      function Q2(e4, n2, i2) {
        ...
        function d4(e5, o3, a3) {
          if ((a3 = n2 ? n2(o3, a3) : a3) !== t2.PipsType.None) {
            var l3 = F2(s3, false);
            l3.className = f3(a3, r2.cssClasses.marker), l3.style[r2.style] = e5 + "%", a3 > t2.PipsType.NoValue && ((l3 = F2(s3, false)).className = f3(a3, r2.cssClasses.value), l3.setAttribute("data-value", String(o3)), l3.style[r2.style] = e5 + "%", l3.innerHTML = String(i2.to(o3)));
          }
        }
      ...
      Tt();
      var zt = { destroy: jt, steps: Dt, on: mt, off: gt, get: Mt, set: kt, setHandle: At, reset: Vt, __moveHandles: function(t3, e4, r3) {
        xt(t3, e4, V2, r3);
...
var P = { name: "Slider", emits: ["input", "update:modelValue", "update", "change"], props: __spreadProps(__spreadValues({}, { value: { validator: function(t) {
  ...
  const l = function(r, n, i) {
    ...
    return { classList: computed(function() {
      ...
      }), s2.value !== "always" && (t.target += " ".concat(s2.value === "drag" ? t.tooltipDrag : t.tooltipFocus)), u2.value === "horizontal" && (t.tooltip += l2.value === "bottom" ? " ".concat(t.tooltipBottom) : " ".concat(t.tooltipTop)), u2.value === "vertical" && (t.tooltip += l2.value === "right" ? " ".concat(t.tooltipRight) : " ".concat(t.tooltipLeft)), t;
    }) };
  }(a), f2 = y(a, 0, { value: l.value, classList: c2.classList }), d2 = function(a2, s2, l2) {
    var c3 = toRefs(a2), p2 = c3.orientation, f3 = c3.direction, d3 = c3.tooltips, h2 = c3.step, m2 = c3.min, v2 = c3.max, g2 = c3.merge, b2 = c3.id, y2 = c3.disabled, S2 = c3.options, P2 = c3.classes, E = c3.format, N = c3.lazy, C = l2.value, O = l2.initialValue, k = l2.tooltipsFormat, V = l2.tooltipsMerge, A = l2.classList, M = ref(null), j = ref(null), U = ref(false), D = computed(function() {
      var t = { cssPrefix: "", cssClasses: A.value, orientation: p2.value, direction: f3.value, tooltips: !!d3.value && k.value, connect: "lower", start: u(C.value) ? m2.value : C.value, range: { min: m2.value, max: v2.value } };
      return h2.value > 0 && (t.step = h2.value), Array.isArray(C.value) && (t.connect = true), t;
    }), L = computed(function() {
      ...
      j.value = x.create(M.value, Object.assign({}, D.value, S2.value)), d3.value && T.value && g2.value >= 0 && V(M.value, g2.value, " - "), j.value.on("set", function(t) {
        var e = z();
        s2.emit("change", e), N.value && H(e);
      }), j.value.on("update", function(t) {
        if (U.value) {
          var e = z();
          T.value && w(C.value, e) || !T.value && C.value == e ? s2.emit("update", e) : N.value || H(e);
        }
      }), M.value.querySelectorAll("[data-handle]").forEach(function(t) {
        t.onblur = function() {
          A.value.focused.split(" ").forEach(function(t2) {
    ...
    return onMounted(q), onUnmounted(R), watch(T, B, { immediate: false }), watch(m2, B, { immediate: false }), watch(v2, B, { immediate: false }), watch(h2, B, { immediate: false }), watch(p2, B, { immediate: false }), watch(f3, B, { immediate: false }), watch(d3, B, { immediate: false }), watch(g2, B, { immediate: false }), watch(E, B, { immediate: false, deep: true }), watch(S2, B, { immediate: false, deep: true }), watch(P2, B, { immediate: false, deep: true }), watch(C, function(t) {
      u(t) ? F(m2.value, false) : (T.value && !w(t, z()) || !T.value && t != z()) && F(t, false);
    }, { deep: true }), { slider: M, slider$: j, isRange: T, sliderProps: L, init: q, destroy: R, refresh: B, update: F, reset: function() {
      H(O.value);
    } };
